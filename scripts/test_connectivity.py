...
逐层验证（任一层失败都明确报出来，不静默跳过）：
  ...
  5. 统计接口（/stats）
  6. 非流式问答（/qa）→ 检查 answer / sources / degraded / degrade_reason / query_log_id
  7. 用返回的 query_log_id 反查 /query-logs，确认能**精确定位**（验证本次修复）
  8. 用该 id 提交反馈（/feedback），确认闭环（验证本次修复）
...
def main():
    ...
    try:
        ...
        st, body, _ = http("POST", API + "/qa",
                           {"question": "STM32F103C8T6 的系统时钟最高频率是多少？",
        ...
        rec("响应含 degraded 字段（本轮新增）", "degraded" in j,
            f"degraded={j.get('degraded')} reason={j.get('degrade_reason')!r}")
        rec("响应含 query_log_id 字段（本轮新增）", "query_log_id" in j, f"query_log_id={j.get('query_log_id')}")
    ...
    if log_id:
        try:
            ...
            if hit:
                print("      该日志: question=%r degraded=%s llm_used=%s sources_count=%s" % (
                    (hit[0].get("question") or "")[:40], hit[0].get("degraded"),
                    hit[0].get("llm_used"), hit[0].get("sources_count")), flush=True)
        except Exception as e:
