...
TEST_QUESTIONS = [
    ("个人信息保护法第一条说了什么", "保护个人信息权益", "法律法规"),
    ("个人信息的定义是什么", "电子或者其他方式", "法律法规"),
    ("处理个人信息要遵循什么原则", "合法、正当、必要", "法律法规"),
    ("第二十八条说的是什么", "敏感个人信息", "法律法规"),
    ("第六十六条是什么", "责令改正", "法律法规"),
    ("处理敏感个人信息需要什么条件", "单独同意", "法律法规"),
    ("什么是API调用", "API", "API学习"),
    ("如何配置环境变量", "环境变量", "API学习"),
    ("什么是知识图谱", "知识图谱", "知识库"),
    ("什么是RAG", "检索增强", "知识库"),
...
def run_test():
    ...
    print("\n[1/5] 准备测试文件...")
    tmp = tempfile.mkdtemp(prefix="real_cft_")
    ...
    for name, (path, required) in REAL_FILES.items():
        if os.path.exists(path):
            ...
            if name == "PDF":
                created = os.path.join(tmp, "pdf_test.pdf")
                create_pdf(created)
                ingest_files[name] = created
                print(f"  {name}: {created} (创建测试文件)")
            elif name == "DOCX":
                created = os.path.join(tmp, "docx_test.docx")
                create_docx(created)
                ingest_files[name] = created
                print(f"  {name}: {created} (创建测试文件)")
            else:
    ...
    for qi, (q, kw, domain) in enumerate(TEST_QUESTIONS, 1):
        ...
        if top1:
            ...
            print(f"\n  {icon} 测试 {qi}: [{domain}] {q}")
            print(f"    Top-1: [{top1_doc}] {top1.content[:50]}...")
        else:
            verdict = "FAIL"
            print(f"\n  [FAIL] 测试 {qi}: [{domain}] {q}")
            print(f"    无检索结果")
    ...
    print(f"\n{'#':<4} {'领域':<10} {'问题':<30} {'Top-1来源':<15} {'判定':<8}")
    print("-" * 70)
    for r in results:
        print(f"{r['qi']:<4} {r['domain']:<10} {r['question']:<30} {r['top1_doc']:<15} {r['verdict']:<8}")
    ...
    for r in results:
        for d in r["hit_docs"]:
            all_hit_docs.add(d)
    print(f"\n  所有 Top-5 出现的来源: {sorted(all_hit_docs)}")
