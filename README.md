...
面向 STM32 MCU 数据手册的检索增强生成（RAG）问答服务。支持多格式文档入库、
向量与关键词双重检索、四级密级权限过滤，并具备答案缓存、多级降级与链路可观测性。
...
| 自动化测试 | **212 用例全绿** | 接入 CI，含覆盖率门禁 |
| 代码规模 | 6500+ 行 / 42 模块 | 不含测试与脚本 |
...
- **运维闭环缺失**：无优雅关闭（`lifespan`）、无接口限流、健康检查未拆分
  liveness / readiness、数据库表结构靠 `create_all()` 而非 Alembic 迁移
- **测试覆盖 60%**：`access/routes.py` 等 9 个模块仍为 0%，主要是需要重依赖
  （ChromaDB / 模型）的分支
- **前端无自动化测试**：934 行的单文件 SPA 目前仅靠人工与连接冒烟脚本验证
